...
export const Tab = (props) => {
  return (
    <div
      className={`relative flex flex-row items-center justify-between h-full px-4 w-48 ${
        props.active
          ? "ActiveTab bg-night-tab border-l-2 border-t-2 border-r-2 rounded-t border-night-tab-active"
          : `${
              props.suffix ? "TabSuffix" : ""
            } Tab hover:bg-night-tab transition duration-300`
    ...
    >
      <div className="flex flex-row items-center justify-start w-full space-x-3">
        ...
        />
        <span
          className={`w-full relative text-xs overflow-hidden whitespace-nowrap max-w-tab-title ${
            props.active
